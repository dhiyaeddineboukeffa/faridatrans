...
try:
    ...
    for col in ws_arrets.iter_cols(values_only=True):
        route_name = col[1]
        ...
        for idx, cell_val in enumerate(col[2:]):
            if not cell_val:
                continue
            ...
            if stop_id not in nodes:
                nodes[stop_id] = {"name": f"{route_name} (Stop {idx+1})", "lat": lat, "lon": lon, "type": "bus"}
                
            if prev_stop_id is not None and prev_stop_id != stop_id:
                dist_m = calc_dist(prev_lat, prev_lon, lat, lon)
                duration_sec = int(dist_m / (30 * 1000 / 3600))
                edges.append({"u": prev_stop_id, "v": stop_id, "mode": "bus", "route": route_id, "duration": duration_sec, "weight": duration_sec})
...
# Also regenerate stations.json used by the routing engine ProximityEngine
# Only include named stops (exclude coordinate-based stops from arrets file)
STATIONS_PATH = r"backend/routing-service/stations.json"
station_list = [
    {"id": stop_id, **data}
    for stop_id, data in nodes.items()
    if not stop_id.startswith("STOP_")  # exclude unnamed coordinate-based stops
]
...
print(f"   Named stations: {len(station_list)} (written to stations.json)")
